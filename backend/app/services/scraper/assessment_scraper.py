...
class AssessmentScraper(BaseScraper):
    ...
    def sanitize_text(self, text: str) -> str:
        """
        Sanitize text by removing escape characters, extra whitespace, and normalizing spaces.
        """
        if not text:
            return ""
        
        sanitized = text.replace("\n", " ").replace("\t", " ").replace("\r", " ")
        sanitized = sanitized.replace("\xa0", " ").replace("\u200b", "")
    ...
    def extract_assessment_details(
        ...
        for test in tqdm.tqdm(tests, unit=("assessments")):
            ...
            description = self.sanitize_text(container[0].p.text)  # type: ignore
            
            # Additional details for relevant recommendation
            job_levels = None
            if len(container) > 2:
                job_levels = self.sanitize_text(container[1].p.text)  # type: ignore
                
            languages = None
            if len(container) > 3:
                languages = self.sanitize_text(container[2].p.text)  # type: ignore
            
            duration = 0
            if container[-1].p:
                duration_text = container[-1].p.text  # type: ignore
                digits = sorted(list(filter(str.isdigit, duration_text.split())))
                extracted_duration = digits[-1] if digits else "0"
...
"""
    ...
    def handle_job_levels(self, job_levels: Optional[str]) -> str:
        if job_levels is None:
            return ""
        return f"This test is designed for {job_levels} roles."
    
    def handle_remote_testing(self, remote_support: bool) -> str:
        return f"This test {'supports' if remote_support else 'does not support'} remote testing."
    
    def handle_testing_lang(self, languages: Optional[str]) -> str:
        if languages is None:
            return ""
        return f"Test available in language(s): {languages}."
